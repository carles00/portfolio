...
export default function Experience({ref, isVisible}:Props) {
  ...
  return (
    <div ref={ref} className={`row-start-2 flex flex-col p-5 opacity-0 transition-opacity duration-700 ease-in ${isVisible ? "opacity-100" : "opacity-0"}`}>
      <div className="grid grid-cols-2">
        <div
          className={`col-start-1 flex flex-col gap-5 p-5  `}
        >
        ...
        <div
          className={`col-start-2 flex flex-col gap-5 p-5`}
        >
          <span className="flex items-center gap-5 text-4xl font-extrabold">
